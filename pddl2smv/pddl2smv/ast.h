...
namespace std{
	...
	ostream& operator<<(ostream& o, const ast::LTLNode& f);
...
namespace ast{
	...
	class GD{
		private:
			AtomicFormulaTerm* atom;
			std::vector<GD*> conjuncts;
		public:
			GD(AtomicFormulaTerm* atom);
			GD(std::vector<GD*> conjuncts);
			AtomicFormulaTerm* getAtom() const;
			std::vector<GD*> getConjuncts() const;
			
			/* Returns the action obtained by replacing each action arguments with a corresponding constant, according to the correspondences in "valuations"
			 * returns null if valuations is not appropriate (i.e., num of arguments and/or types are not consistent)
			 */
	...
	class InitElement{
	  ...
	  struct LTLNode{
		  ...
		  AtomicFormulaTerm* atom; // Significant only if op=NN
		  ...
		  LTLNode* valuate(std::vector<const Predicate*> groundPredicates);		  
	  };
	  /***************************************/
	...
	class Problem : public Symbol{
		...
		public:
			Problem();
			Problem(const std::string& name, Domain* domain, std::vector<std::string> requirements, std::vector<Constant*> objects, std::vector<InitElement*> init, LTLNode* goal);
			Problem* valuate(std::vector<const Predicate*> groundPredicates) const;
			Domain* getDomain() const;
			std::vector<std::string> getRequirements() const;
			std::vector<Constant*> getObjects() const;
			std::vector<InitElement*> getInit() const;
			LTLNode* getGoal() const;
	};
