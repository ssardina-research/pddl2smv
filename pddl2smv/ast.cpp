...
std::ostream& std::operator<<(std::ostream& o, const ast::LTLNode& f){
	f.print(o);
	return o;
}
...
AtomicFormulaTerm* GD::getAtom() const{
	return atom;
}
...
GD* GD::valuate(map<const Variable*, const Constant*> valuations, vector<const Predicate*> groundPredicates) const{
	if (atom != NULL){// The goal is an atom
		AtomicFormulaTerm* groundAtom = atom->valuate(valuations, groundPredicates);
		if (groundAtom == NULL){
			return NULL;
		}
...
/* Problem */
Problem::Problem() : Symbol("") {};

Problem::Problem(const string& name, Domain* domain, vector<std::string> requirements, vector<Constant*> objects, vector<InitElement*> init, LTLNode* goal) : Symbol(name){
	this -> domain =  domain;
	this -> requirements = requirements;
...
Problem* Problem::valuate(vector<const Predicate*> groundPredicates) const{
	map<const Variable*,const Constant*> empty_map;
	vector<InitElement*> grnd_init_state;
	for (unsigned int i = 0; i < init.size(); i++){
			grnd_init_state.push_back(new InitElement(init[i]->getLiteral()->valuate(empty_map,groundPredicates)));
	}
	
	LTLNode* grnd_goal = goal -> valuate(groundPredicates);
		
	return new Problem("grnd_"+getName(),domain,requirements,objects,grnd_init_state,grnd_goal);
}

LTLNode* Problem::getGoal() const{
	return goal;
}
...
void ast::LTLNode::print(ostream &o) const{ 
	switch (op){
...
LTLNode* LTLNode::valuate(std::vector<const Predicate*> groundPredicates){
	LTLNode* result = new LTLNode();
	if (op == NN){
		map<const Variable*, const Constant*> empty_map;
		result -> op = NN;
		result -> left = NULL;
		result -> right = NULL;
		result->atom = atom->valuate(empty_map, groundPredicates); // LTLNode atoms cam only be ground
	}
	else{
		result -> op = op;
		result -> left = NULL;
		...
		result-> atom = NULL;
		if (left != NULL){
			result -> left = left -> valuate(groundPredicates);			
		}
		if (right != NULL){
			result -> right = right -> valuate(groundPredicates);			
		}
	}
	return result;
}
