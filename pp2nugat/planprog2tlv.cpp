...
void PlanProg2TLV::outputGame(std::ostream& o){
	o << "GAME" << endl;	
	o << "--- NON-DETERMINISTIC ENVIRONMENT PLAYER" << endl;	
	o << "PLAYER_1" << endl;	
	o << ind() << "VAR" << endl;
	o << ind(2) << "dom : environment_module(agent);" << endl;
	o << "--- CONTROLLER PLAYER ENVIRONMENT" << endl;	
	o << "PLAYER_2" << endl;	
	o << ind() << "VAR" << endl;
	o << ind(2) << "agent : system_module(dom);" << endl;
	o << endl;
	o << endl;
	o << "BUCHIGAME PLAYER_2 (agent.last)" << endl;
	o << "-- end of NuGAT planning program encoding" << endl;
}
...
void PlanProg2TLV::outputSystem(const Domain* d, const PlanningProgram* p, std::ostream& o){
	...
	o << ind() << "INIT" << endl;	
	o << ind(2) << "action = start_action & last = FALSE" << endl << endl;
...
void PlanProg2TLV::outputPreconditions(const Domain* d, std::ostream& o, unsigned int indt){
	...
	o << ind(indt+1) << "TRUE : TRUE;" << endl; // Seb
